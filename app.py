...
def init_db():
    ...
    check = conn.execute(
        "SELECT COUNT(*) FROM registros WHERE salida IS NULL"
    ).fetchone()[0]
    ...
    if check == 0:
        for i in range(1, 98):
            ...
            hora = (
                datetime.now()
                - timedelta(minutes=random.randint(10, 480))
            ).strftime("%Y-%m-%d %H:%M:%S")

            conn.execute(
                "INSERT INTO registros (placa, casa, entrada) VALUES (?, ?, ?)",
                (placa, casa, hora)
            )
...
def index():
    ...
    activos_raw = conn.execute(
        "SELECT * FROM registros WHERE salida IS NULL ORDER BY id DESC"
    ).fetchall()
...
def ingreso():
    ...
    if conn.execute(
        "SELECT 1 FROM registros WHERE casa=? AND salida IS NULL",
        (casa,)
    ).fetchone():
        conn.close()
...
def salida(id):
    ...
    conn.execute(
        "UPDATE registros SET salida=? WHERE id=?",
        (ahora, id)
    )
